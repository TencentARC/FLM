...
def make_arrow(root, dataset_root):
    for split in ["val", "train"]:
        ...
        arrow_path = "{dataset_root}/conceptual_caption_{split}_{sub}.arrow"
        write_split(caption_paths, iid2captions,
                    dataset_root, arrow_path, split)


def write_split(caption_paths, iid2captions, dataset_root, arrow_path, split):
    ...
    for sub in subs:
        ...
        with pa.OSFile(
            arrow_path.format(**{'dataset_root': dataset_root,
                                 'split': split,
                                 'sub': sub}), "wb") as sink:
